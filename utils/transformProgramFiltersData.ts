...
import type { ProgramCard, EntityOption, ProgramMap } from '~/types/ProgramCard';
import type {
...
export function transformProgramFiltersData(response: UnionFilterResponse) {
    ...
    programs = response.data.map(({ id, type, attributes, relationships }) => {
      ...
      Object.keys(relationships).forEach((relation) => {
        ...
        if (['faculty', 'level'].includes(relation)) {
          ...
          if (relation === 'level') {
            relationsMap.levels[program.level.value].count = relationsMap.levels[program.level.value].count as number + 1;
          }
      ...
      if (program.type === 'courses') {
        relationsMap.levels['1000'].count = relationsMap.levels[program.level.value].count as number + 1;
      }
      ...
      const programType = program.type as 'courses' | 'programs';
      if (programsMap[programType]) programsMap[programType][program.id] = program;
      return program;
    });
  } catch (error) {
    console.error(error);
...
function makeOptions({ id, type, attributes, relationships }: Entity): EntityOption {
  ...
  const types = {
    ...
    faculties(data: EntityOption) {
      ...
      data.intro_image = attributes.intro_image;
      return data;
    },
    levels(data: EntityOption) {
      ...
      const benefitsString = attributes?.properties?.filter_benefits;

      data.filterBenefits = typeof benefitsString === 'string' && benefitsString ? benefitsString.split('\n').filter((el) => Boolean(el.trim())) : [];
...
function getProgramCardTemplate(type: 'programs' | 'courses', id: string, attributes: ProgramAttributesObject | CourseAttributesObject): ProgramCard {
  ...
  return {
    ...
    isPopular: false,
    partner: Boolean(attributes.properties?.partner_show),
