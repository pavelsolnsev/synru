...
const formsTraining = hasEntity && !['branches', 'branch', 'branches-new'].includes(routeName)
...
async function getFormsTrainingByEntity() {
  ...
  const { data, error } = await useFetch(`/api/programs/getFormsByProgramId/${entity.id}`, {
    deep: false,
    watch: false,
  });
...
async function getFormsTraining() {
  const { data, error } = await useFetch('/api/forms/getForms', {
